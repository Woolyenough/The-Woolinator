...
class Misc(commands.Cog, name='Miscellaneous', description='Uncategorised stuff'):
            ...
            for file in filenames:
                if file.endswith('.py'):
                    ...
                    try:
                        ...
                        log.warn(f"Couldn't read file '{file_path}'", exc_info=e)
                        continue
    ...
    async def prefix(self, ctx: Context, new_prefix: commands.Range[str, 1, 4] = ''):
        if new_prefix:
            ...
            if personal_prefix is None: personal_prefix = 'None set' if guild_prefix != 'None set' else self.bot.default_prefix
